module Page.Directory exposing (Model, Msg, init, toSession, update, view)
...
init : Session.Session -> Maybe String -> ( Model, Cmd Msg )
init session maybeSection =
    case maybeSection of
        Just encodedSection ->
            case Url.percentDecode encodedSection of
                Just section ->
                    ( initialModel session
                    , Task.attempt AttemptedScroll
                        (Dom.getElement section
                            |> Task.andThen
                                (\info -> Dom.setViewport 0 info.element.y)
                        )
                    )
...
type Msg
    = HoveredLink String
    | UnHoveredLink
    | AttemptedScroll (Result Dom.Error ())
